...
const BROADCAST_AFTER_SUBSCRIBE_REPLAY_ID = -1;
...
export default class Chat extends LightningElement {
    ...
    subscribeOnChatEvent() {
        const receiveMessageHandler = (response) => {
            this.receivedMessageId = response.data.payload[EVENT_MESSAGE_ID_FIELD.fieldApiName];
        };

        subscribe(CHANNEL_NAME, BROADCAST_AFTER_SUBSCRIBE_REPLAY_ID, receiveMessageHandler).then(response => {
